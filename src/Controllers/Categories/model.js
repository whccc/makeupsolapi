...
module.exports = {
  ...
  async MDFind() {
    // Get Categories
    const objRes = await Categories.find({}, { strName: 1 }).sort({
      $natural: -1,
    });
    return objRes;
  },
  ...
  async MDFindIdUpdate({ _id, strText }) {
    const objRes = await Categories.findByIdAndUpdate(
      { _id },
      { strName: strText }
    );
    return objRes;
  },
  async MDDeleteCategory({ _id }) {
    try {
      await Categories.findByIdAndRemove({ _id });
    } catch (Error) {
      console.log(Error);
    }
  },
